...
- **پروتکل‌های متعدد** - Reality (VLESS)، Trojan، Hysteria2، TrustTunnel، WireGuard (مستقیم و wstunnel)، تونل‌های DNS (dnstt + Slipstream)
...
**نام مستعار سرویس‌ها:** `conduit`←psiphon-conduit، `singbox`←sing-box، `wg`←wireguard، `dns/dnstt/slip`←dnstunnel
...
تست Reality، Trojan، Hysteria2، WireGuard، dnstt و Slipstream. گزارش pass/fail/skip برای هر پروتکل.
...
پروتکل‌های موجود: `reality`، `trojan`، `hysteria2`، `wireguard`، `dnstt`، `slipstream`، `psiphon`، `tor`
