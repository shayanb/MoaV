...
### Added
- **One-tap MahsaNG import in the client guide** — every user bundle's `README.html` now opens with a **"MahsaNG — Import everything at once"** section (EN + FA): a click-to-copy V2Ray **subscription** (base64 of the compatible share-links — Reality, CDN, XHTTP, Trojan, Shadowsocks, Hysteria2, IPv4 + IPv6) that the user pastes once into MahsaNG v16 to import every proxy protocol in one step. Mirrors `moav user mahsanet`; the DNS tunnels and GooseRelay (configured in their own MahsaNG tabs) are intentionally excluded. Auto-hidden when the bundle has no compatible configs.
