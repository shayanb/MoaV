...
### Fixed
- **Domainless bootstrap failed with "DOMAIN is required when TLS-cert protocols are enabled" right after the operator opted into domainless mode** — `scripts/bootstrap.sh` requires `DOMAIN` if any of 6 vars (`ENABLE_TROJAN` / `ENABLE_HYSTERIA2` / `ENABLE_DNSTT` / `ENABLE_SLIPSTREAM` / `ENABLE_MASTERDNS` / `ENABLE_TRUSTTUNNEL`) is true. The moav.sh domainless setter — both the inline path during interactive bootstrap and the standalone `moav domainless` command — was only disabling 5: `ENABLE_MASTERDNS` was added in 1.8.0 but never propagated to the disable list, so the bootstrap container errored out immediately after the operator said yes to domainless mode (MasterDNS is default-on since 1.8.0). Both disable loops now include `ENABLE_MASTERDNS`. The user-facing "Services that require a domain" prompt now mentions MasterDNS in the DNS-tunnels line, and the "Services that work without a domain" list adds the two protocols that already worked but weren't listed (XHTTP, Shadowsocks-2022) — same in the standalone `moav domainless` confirmation screen, and the post-confirm success line. `scripts/bootstrap.sh`'s own error message likewise expanded to list all 6 cert-requiring vars (it was missing `ENABLE_SLIPSTREAM` and `ENABLE_MASTERDNS`), and its "Running in domainless mode (…)" line is now derived from the actual `ENABLE_*` flags rather than a hand-curated literal so the listed protocols stay accurate as toggles flip
- **`ENABLE_SNOWFLAKE=false` / `ENABLE_CONDUIT=false` could still start the donation services via default/all profile expansion** (issue [#106](https://github.com/shayanb/MoaV/issues/106), thanks @vibecodegits / Codex) — `DEFAULT_PROFILES` is a docker-compose construct that doesn't know about MoaV's `ENABLE_*` flags. Several code paths (domainless-mode bootstrap setter, interactive "all enabled services" path, monitoring-skip fallbacks) hard-coded a literal profile list that always appended `conduit` and `snowflake`. Result: a fresh install (or any path that hit one of those setters) ended up with `DEFAULT_PROFILES="… conduit snowflake"`, and `moav start` happily expanded each entry to `--profile X` regardless of the matching `ENABLE_*` flag — so the Snowflake relay actively relayed Tor traffic on installs where the operator had explicitly set `ENABLE_SNOWFLAKE=false`. Fix is a single source of truth: a new `profile_enabled` / `derive_enabled_profiles` / `filter_disabled_profiles` helper trio that maps each docker-compose profile to its underlying `ENABLE_*` flag(s). Every hard-coded literal site now calls `derive_enabled_profiles` to populate `DEFAULT_PROFILES`, and `moav start` filters whatever `DEFAULT_PROFILES` says through `filter_disabled_profiles` so existing installs with a stale literal self-heal on the next start (with a one-line "Skipping disabled profiles: …" note). `moav start all` no longer falls through to `docker compose --profile all up` — `all` now expands to the derived enabled set, so `--profile all` keeps its enumeration semantics (build / logs / down still hit every service) without `up` starting disabled ones. Explicit-name `moav start snowflake` when `ENABLE_SNOWFLAKE=false` now prompts with three choices: (1) flip `ENABLE_SNOWFLAKE=true` in `.env` and start, (2) skip, or (3) start once without persisting. Non-interactive shells get the safe default (skip). `--force` bypasses the prompt. The "fall back to donation-only" branch (which forced `conduit`/`snowflake` on when no enable flags matched, exactly the bug pattern) is removed — empty derivation now errors out instead. `snowflake-exporter` moved from `[monitoring, all]` to `[snowflake, all]` so the Snowflake Grafana panels don't keep emitting stale numbers when the relay is off; it's still reachable via `--profile all` for enumeration ops. **Action for affected installs:** if your `DEFAULT_PROFILES` literal in `.env` already includes `snowflake` or `conduit` despite `ENABLE_*=false`, the new filter drops them on the next `moav start` (and prints which it dropped). To make it permanent, edit `DEFAULT_PROFILES` in `.env` to drop the stale entries, or re-run the interactive selection
